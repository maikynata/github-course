...
# linhas de teste 2141 1 5 21591
...
OUTPUT_DIR='/data/DADOS_GRID/BFAST10/tif_1/'
...
runBfast = function(data, ncores) {
  ...
  cat("Running rbind", '\n')
  bfastResult <- foreach(combine = data.frame, bfastResultList) %dopar%

  # parApply(cl = clusterPool, 'rbind', 1, bfastResultList)
  cat("Running rm bfastResultList", '\n')
  rm(bfastResultList)
  gc(verbose=FALSE)
...
cat("Running function saveImage", '\n')
